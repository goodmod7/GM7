...
export async function buildDesktopAccountSnapshot(input: BuildDesktopAccountSnapshotInput) {
  ...
  return {
    ...
    billing: {
      ...
      localAiPlan: input.user.subscriptionStatus === 'active' ? 'plus' : 'free',
      freeLocalTaskLimit: input.user.subscriptionStatus === 'active' ? null : 5,
      visionBoostIncluded: input.user.subscriptionStatus === 'active',
    },
    currentDevice,
